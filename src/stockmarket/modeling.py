...
def train_model(feature_frame: pd.DataFrame,test_fraction: float=0.2,random_state: int=42)->ModelResult:
    ...
    if split<20 or len(feature_frame)-split<5: raise ValueError("Training and test windows are too small")
    train_frame=feature_frame.iloc[:split]; test_frame=feature_frame.iloc[split:]
    validation_model=fit_model(train_frame); prediction=validation_model.predict(test_frame)
