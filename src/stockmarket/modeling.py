...
def model_feature_columns(feature_frame: pd.DataFrame) -> list[str]:
    extras = [
        ...
        if column.startswith(MODEL_CONTEXT_PREFIXES)
...
def _validate_training_frame(feature_frame: pd.DataFrame, minimum_rows: int = 20) -> list[str]:
    feature_columns = model_feature_columns(feature_frame)
    ...
    if len(feature_frame) < minimum_rows:
        raise ValueError(f"At least {minimum_rows} feature rows are required for training")
...
def momentum_prediction(features: pd.DataFrame) -> np.ndarray:
    ...
    if missing:
        raise ValueError(f"Momentum features are missing columns: {', '.join(sorted(missing))}")
...
def evaluate_predictions(y_true: pd.Series, prediction: np.ndarray) -> dict[str, float]:
    ...
    if len(actual) != len(predicted) or len(actual) == 0:
        raise ValueError("Actual and predicted returns must have the same non-zero length")
    residual = actual - predicted
    strategy_returns = np.where(predicted > 0.0, actual, 0.0)
    return {
...
def fit_model(feature_frame: pd.DataFrame, ridge_penalty: float = 1e-3, momentum_weight: float = 0.30) -> ModelResult:
    feature_columns = _validate_training_frame(feature_frame)
    if ridge_penalty < 0:
    ...
    return ModelResult(coefficients, mean, scale, ridge_penalty, {}, feature_columns, momentum_weight)
...
def train_model(feature_frame: pd.DataFrame, test_fraction: float = 0.2, random_state: int = 42, purge: int = 1) -> ModelResult:
    ...
    train_end = int(len(feature_frame) * (1.0 - test_fraction))
    ...
    prediction = validation_model.predict(test_frame)
    metrics = evaluate_predictions(test_frame["target_return"], prediction)
