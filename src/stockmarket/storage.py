...
class Store:
    ...
    def _ensure_orders_schema(connection:sqlite3.Connection)->None:
        ...
        if "realized_pnl" not in columns: connection.execute("ALTER TABLE orders ADD COLUMN realized_pnl REAL DEFAULT 0")
        if "reason" not in columns: connection.execute("ALTER TABLE orders ADD COLUMN reason TEXT DEFAULT 'manual'")
    ...
    def orders(self)->list[dict]:
        with sqlite3.connect(self.path) as connection:
            ...
            return [dict(row) for row in connection.execute("SELECT * FROM orders ORDER BY id DESC").fetchall()]
