...
def _rsi(close: pd.Series, window: int = 7) -> pd.Series:
    ...
    avg_gain = _wilder_ema(gain, window)
    avg_loss = _wilder_ema(loss, window)
...
def build_features(
    ...
    frame["sma_5"] = close.rolling(5, min_periods=5).mean()
    frame["sma_10"] = close.rolling(10, min_periods=10).mean()
    frame["rsi_7"] = _rsi(close, 7)
    ...
    frame["atr"] = _wilder_ema(_true_range(frame), 14)
    frame["volatility_10"] = close.pct_change().rolling(10, min_periods=10).std()
    frame["volume_chg"] = pd.to_numeric(frame["Volume"], errors="coerce").pct_change()
    for lag in range(1, 7):
        frame[f"close_lag_{lag}"] = close.shift(lag)

    required_columns = list(FEATURE_COLUMNS)
    if include_target:
        targets = build_forward_targets(close, horizon, round_trip_cost)
        for column in TARGET_COLUMNS:
            frame[column] = targets[column]
        required_columns.extend(TARGET_COLUMNS)
