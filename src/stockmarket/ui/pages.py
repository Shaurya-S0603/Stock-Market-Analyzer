...
def render_dashboard(available: dict[str, SymbolAnalysis], portfolio: PaperPortfolio, prices: dict[str, float]) -> None:
    ...
    st.dataframe(signal_table(available), width="stretch", hide_index=True)
...
def render_model_health(available: dict[str, SymbolAnalysis], analysis_service: AnalysisService) -> None:
    st.subheader("Model metrics by symbol")
    st.dataframe(model_table(available), width="stretch", hide_index=True)
    st.caption("These metrics use historical holdout data. They are diagnostics, not evidence of future profitability.")
    ...
    try:
        ...
        st.info(f"Walk-forward validation skipped: {exc}")
...
def render_trading(available: dict[str, SymbolAnalysis], portfolio: PaperPortfolio, portfolio_service: PortfolioService, store: Store, prices: dict[str, float]) -> None:
    ...
    with left:
        ...
        if submitted:
            try:
                fill = portfolio_service.execute(symbol,side,int(quantity),float(price)); st.success(f"Recorded {fill.side} {fill.quantity} {symbol} at ${fill.price:,.2f}."); st.rerun()
        ...
        if position and position.quantity > 0 and st.button(f"Close all {symbol}",use_container_width=True):
            try:
                fill = portfolio_service.execute(symbol,"sell",position.quantity,reference_price,reason="manual_close"); st.success(f"Closed {fill.quantity} shares of {symbol} at ${fill.price:,.2f}."); st.rerun()
            except TradingError as exc: st.error(str(exc))
    with right:
        st.subheader("Open positions")
        positions = positions_table(portfolio,prices)
        st.info("No open paper positions.") if positions.empty else st.dataframe(positions,width="stretch",hide_index=True)
    st.subheader("Order history")
    orders = orders_table(store.orders())
    st.info("No paper orders recorded yet.") if orders.empty else st.dataframe(orders,width="stretch",hide_index=True)
...
def render_backtest(available: dict[str, SymbolAnalysis], analysis_service: AnalysisService, request: AnalysisRequest, starting_cash: float) -> None:
    ...
    cols = st.columns(3); cols[0].metric("Total return",f"{summary['total_return_pct']:.2f}%"); cols[1].metric("Max drawdown",f"{summary['max_drawdown_pct']:.2f}%"); cols[2].metric("Trades",str(summary["trades"]))
